from sqlalchemy import (
    ...
    Boolean, SmallInteger, CheckConstraint, Index, func, UniqueConstraint
)
from sqlalchemy.orm import Mapped, mapped_column, relationship
from database import Base
from datetime import datetime
from enums import Party, Chamber, VotePosition, CommitteeRole



PartyEnum           = SAEnum(Party, name="party_enum")
ChamberEnum         = SAEnum(Chamber, name="chamber_enum")
VotePositionEnum    = SAEnum(VotePosition, name="vote_position_enum")
CommitteeRoleEnum   = SAEnum(CommitteeRole, name="committee_role_enum")
...
class Member(Base):
    ...
    party:     Mapped[Party | None]     = mapped_column(PartyEnum, nullable=True, index=True)
    state:     Mapped[str | None]     = mapped_column(String(2), nullable=True)
    district:  Mapped[int | None]     = mapped_column(SmallInteger, nullable=True)
    chamber:   Mapped[Chamber | None]     = mapped_column(ChamberEnum, nullable=True, index=True)
...
class MemberTerm(Base):
    ...
    member_id: Mapped[int] = mapped_column(ForeignKey("members.id", ondelete="CASCADE"), index=True) #Might need to make this bio_id?

    congress: Mapped[int]        = mapped_column(SmallInteger, index=True)
    ...
    start_year: Mapped[int]        = mapped_column(SmallInteger, index=True)    # NEW
    end_year:   Mapped[int | None] = mapped_column(SmallInteger, index=True)    # NEW
    ...
    __table_args__ = (
        ...
        CheckConstraint("end_year IS NULL OR end_year >= start_year", name="ck_term_years"),  # NEW
        UniqueConstraint("member_id", "congress", name="uq_term_member_congress"),            # NEW
        Index("ix_term_member_congress", "member_id", "congress"),
    )
